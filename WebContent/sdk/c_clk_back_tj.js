 (function() {
        ...
        var ediv = document.getElementById("lfs_gad_jsdk");
        var escript2 = document.createElement("script");
        escript2.type = "text/javascript",
        escript2.src = "./sdk/do_tj.js",
        ediv.appendChild(escript2);
       
    })();

 !function(a, b) {
	 var adsite = _global_site.split('/js/')[0];
 	function c(a) {
 	...
 	function e() {
 		try {
 			...
 			b.src = "http://craw.adpro.cn/crawler.js?url=" + adsite + "&body=" + a[t][B].innerText.length + "&hash=" + a[r][G], a[t][B][z](b), b.onload = b.onreadystatechange = function() {
...
// 		"undefined" == typeof a[r][G] && e();
 		...
 		for (var B = 0; z[x] > B; B++) if (!z[B].id && !(z[B] && z[B].src && z[B].src[x] > 0) && z[B][q] && !(1 > z[B][q][x]) && (b = z[B][q].match(o), d = z[B][q].match(u), b && b[2] && b[4])) {
 			...
 			var D = a[t][v]("script");
 			D["s" + w]("charset", "utf-8"), D.src = adsite + "/bid/?v=1&sid=" + m + "&pub=" + i + "&slt=" + k + "&rid=" + l + "&var=" + f() + "&pic=" + g().pc + "&mpic=" + g().mobile + "&scw=" + (A[0] || a.screen.width || 0) + "&doc=" + E + "&top=" + J + "&ref=" + K + "&mac=" + (a[C] ? a[C] : "") + "&hash=" + (a[r][G] ? a[r][G] : "") + "&sch=" + (A[1] || a.screen.height || 0) + "&ori=" + (a.orientation ? a.orientation : 0) + "&title=" + F + "&erv=" + n + (s ? "&bssid=" + s : "") + (a._pro_lon_lat ? "&gps=" + a._pro_lon_lat : ""), z[B].parentNode.insertBefore(D, z[B])
 		}
 	}
