...
function(a, b) {
	...
	function i() {
		...
		for (var B = 0; z[x] > B; B++) if (!z[B].id && !(z[B] && z[B].src && z[B].src[x] > 0) && z[B][q] && !(1 > z[B][q][x]) && (b = z[B][q].match(o), d = z[B][q].match(u), b && b[2] && b[4])) {
			...
			var D = a[t][v]("script");
			D["s" + w]("charset", "utf-8"), D.src = j + "/bid/?v=1&sid=" + m + "&pub=" + i + "&slt=" + k + "&rid=" + l + "&var=" + f() + "&pic=" + g().pc + "&mpic=" + g().mobile + "&scw=" + (A[0] || a.screen.width || 0) + "&doc=" + E + "&top=" + J + "&ref=" + K + "&mac=" + (a[C] ? a[C] : "") + "&hash=" + (a[r][G] ? a[r][G] : "") + "&sch=" + (A[1] || a.screen.height || 0) + "&ori=" + (a.orientation ? a.orientation : 0) + "&title=" + F + "&erv=" + n + (s ? "&bssid=" + s : ""), z[B].parentNode.insertBefore(D, z[B])
		}
	}
